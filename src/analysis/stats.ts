...
import { extractTokensFromBody } from '../providers/response-format.js';
...
export async function getSessionStats(sessionId: string, store: EventStore): Promise<SessionStats> {
  ...
  while (i < events.length) {
    ...
    if (event.type === 'request') {
      ...
      if (response) {
        const turn = buildTurnStats(turnIndex, event, response);
        turns.push(turn);
...
function buildTurnStats(
  ...
  if (response.streamed && response.event.type === 'stream_end') {
    ...
    // Extract tokens from response body (any provider format)
    const tokens = extractTokensFromBody(data.body);
    if (tokens) {
